...
| 12:30 - 1:00 | Section 4: MCP (Model Context Protocol) | 30 min |
| 1:00 - 1:40 | Section 5: Hooks and Rules | 40 min |
| 1:40 - 2:10 | Section 6: Task Board, Scheduled Tasks & Wrap-Up | 30 min |
...
### 3.1 Building a Streamlit App Locally

**Scenario:** The store operations team needs a real-time dashboard showing inventory levels, out-of-stock items, and spoilage alerts by department.

**How it works:** CoCo uses the `/build-app` skill to scaffold Streamlit apps. It asks what you want to build, recommends a framework (Streamlit-in-Snowflake for dashboards, Snowflake App Runtime for full-stack web apps), confirms your choice, then generates a working project with Snowflake connection wiring, sample data, and a local dev server.

**Hands-on Exercise:**

1. In the Agent tab, type: `/build-app create an app for a store dashboard`
...
2. CoCo will ask what the app should do. Provide a detailed description that includes:
   ...
   Example prompt:
   > "This should be for store managers to use. It should include a store selector and date range picker. It should show KPIs for total revenue, shrinkage rate, out-of-stock count, avg basket size. I would like to see a bar chart of revenue by department and a table of the top 10 items approaching expiration date. Use data from RETAIL_DB.SALES and RETAIL_DB.INVENTORY schemas."

3. CoCo recommends **Streamlit-in-Snowflake** as the framework (since this is a pure view/explore dashboard with no user actions). Confirm the selection. Note: this choice means CoCo scaffolds a Streamlit app designed for eventual deployment to Snowflake — during development, it runs **locally** on your laptop and connects to Snowflake for data. Section 3.2 covers the actual deployment to Snowflake.

4. CoCo discovers the tables in your schemas, inspects their columns, scaffolds the project with queries against your real data, starts the local dev server, and opens a browser preview at `localhost:8501`.

5. The initial scaffold may have errors (wrong table names, connection issues, incompatible Streamlit version, etc.). Ask CoCo to fix them:
   > "Test and validate the streamlit app works without errors and that selecting a store or date range updates the data."

   CoCo will open the browser, identify any runtime errors, fix the code (correcting table/column references, connection approach, package versions), and verify the app renders correctly with live data.

6. Interact with the dashboard — change the store selector and date range and verify the KPIs, bar chart, and expiration table all update with different values for each store.

7. **Iterate on the app with natural language.** Building an app with CoCo is iterative — once the base dashboard works, you can keep adding features by describing what you want. CoCo will modify the existing code, query new tables as needed, and hot-reload the preview.
   ...
   - Hot-reload the browser preview so you see the result immediately
...
**Key points:**
- `/build-app` handles the full flow: framework recommendation → scaffold → local preview
- The initial scaffold uses synthetic data so the app works immediately — you layer in real tables after
- CoCo uses raw Snowflake cursor with bind parameters (`?`) for parameterized queries — this ensures filters properly update the data
- Local development = instant iteration with hot-reload
- Full access to any Python library (altair, pandas, plotly)
- Connects to Snowflake using your local credentials (set via `SNOWFLAKE_DEFAULT_CONNECTION_NAME` environment variable)

### 3.2 Deploying to Snowflake (Streamlit in Snowflake)

**Scenario:** The dashboard is ready — now make it available to all 200 store managers via Snowflake without them needing Python installed.

| Aspect | Local | Deployed (SiS) |
|--------|-------|-----------------|
| Runtime | Your laptop | Snowflake infrastructure |
| Packages | Any pip package | Snowflake Anaconda channel (max Streamlit 1.52) |
| Auth | Your credentials | Snowflake session (SSO) |
| Access | Only you | Any role you grant |
| Data | Via `snowflake.connector` | Native Snowpark `get_active_session()` |
| Iteration | Hot-reload | Re-upload files to stage |

**Hands-on Exercise:**

1. Ask CoCo to adapt and deploy the app. **Use your initials as a prefix** for the app name since multiple workshop participants will be deploying to the same account:

   > "Adapt the store dashboard for deployment to Streamlit in Snowflake. Use Snowpark for data access instead of snowflake.connector. Name the app '[YOUR_INITIALS]_Store Dashboard'. Test and verify it is running with no errors after it is deployed."

   For example: `CL_Store Dashboard`, `JD_Store Dashboard`, etc.

   CoCo handles the entire migration automatically — it rewrites the connection layer to use Snowpark's `get_active_session()`, adjusts queries for Snowpark's SQL API, fixes any Streamlit features that aren't compatible with the SiS runtime (which runs an older Streamlit version than your local install), creates a stage, uploads the files, creates the Streamlit object, and then opens the deployed app to test for errors. If it finds issues, it fixes them and re-uploads until the app runs cleanly.

2. Once CoCo confirms the app is running, navigate to it yourself in Snowsight: **Projects** → **Streamlit** → find your named app (e.g. "CL_Store Dashboard"). Test both tabs and confirm data loads.

---

## Section 4: MCP — Model Context Protocol (30 min)
...
### 4.2 Adding and Managing MCP Servers
...
### 4.3 Using MCP Tools in Practice
...
## Section 5: Hooks and Rules (40 min)
...
### 5.2 Rules (AGENTS.md / Project Instructions)
...
## Section 6: Task Board, Scheduled Tasks & Wrap-Up (30 min)

### 6.1 Scheduled Tasks with CoCo
...
### 6.2 The Task Board — Project Task Tracking
...
### 6.3 Scheduled Tasks & Automation
...
### 6.4 Session Management for Continuity
...
4. **MCP** — Connect supplier APIs, weather data, POS systems directly into your workflow
5. **Streamlit** — Prototype locally, deploy to all 200 store managers via Snowflake
